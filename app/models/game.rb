  ...
  def addScore(event, rank, person)
     ...
     if isTimeBased()
          ...
          if (scores.size == 0)
            ...
            lastGameTime = scores[0].created_at
          end
    ...
    if shouldEverybodyWin()
        people = Person.all
        people.each{|person| person.addScore(event:nil, rank:rank, game:self, points:@points)}
        score = scores[-1]  # return the score that was just added to your list
    end
